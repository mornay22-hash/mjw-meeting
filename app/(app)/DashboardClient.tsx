...
export default function DashboardClient({
  ...
  const todayMeetings = meetings.filter(m => new Date(m.meeting_date).toDateString() === todayStr)
  const upcomingMeetings = meetings.filter(m => new Date(m.meeting_date) > now && new Date(m.meeting_date).toDateString() !== todayStr)
    ...
    <div className="space-y-8">
      ...
      {todayMeetings.length > 0 && (
        <Section title="Today" count={todayMeetings.length}>
          {todayMeetings.map(m => <MeetingCard key={m.id} meeting={m} onDelete={deleteMeeting} />)}
        </Section>
      )}
      ...
      {upcomingMeetings.length > 0 && (
        <Section title="Upcoming" count={upcomingMeetings.length}>
          {upcomingMeetings.map(m => <MeetingCard key={m.id} meeting={m} onDelete={deleteMeeting} />)}
        </Section>
      )}
...
function MeetingCard({ meeting, onDelete }: { meeting: Meeting; onDelete: (id: string, e: React.MouseEvent) => void }) {
    ...
    <Link href={href} style={{ textDecoration: 'none' }} className="block">
      ...
      >
        ...
        <button
          ...
          style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--slate-dk)', padding: 4, flexShrink: 0, display: 'flex', borderRadius: 6, transition: 'color 0.15s' }}
          onMouseEnter={e => (e.currentTarget.style.color = '#e05c5c')}
          onMouseLeave={e => (e.currentTarget.style.color = 'var(--slate-dk)')}
        >
